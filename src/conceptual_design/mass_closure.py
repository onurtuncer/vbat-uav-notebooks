...
def write_sizing_result(
    ...
    """
    Write out/sizing_result.yaml -- the handoff file consumed by
    downstream notebooks (airfoil selection, wing design, etc.).

    Contains only the numbers a subsequent design phase needs as inputs.
    The full SizingResult object (with convergence history) stays in
    memory; only the settled design-point values are persisted.

    Parameters
    ----------
    result       : converged SizingResult from run_sizing_loop()
    smd_WS       : design wing loading from Size Matching Diagram [N/m^2]
    smd_TW       : design T/W from Size Matching Diagram          [-]
    ...
    path = out_dir / "sizing_result.yaml"

    data = {
        # -- MTOW and mass breakdown --
        "MTOW_kg":          round(result.m_total_kg, 4),
        "m_battery_kg":     round(result.m_battery_kg, 4),
        "m_payload_kg":     round(result.m_payload_kg, 4),
        "m_structure_kg":   round(result.m_structure_kg, 4),
        "m_propulsion_kg":  round(result.m_propulsion_kg, 4),
        "m_avionics_kg":    round(result.m_avionics_kg, 4),
        "m_misc_kg":        round(result.m_misc_kg, 4),
        "m_wing_kg":        round(result.m_wing_kg, 4),
        "batt_fraction":    round(result.batt_fraction, 5),
        ...
        "TW_design":        round(smd_TW, 5),
        "S_wing_m2":        round(result.wing.S_wing, 5),
        "b_wing_m":         round(result.wing.b_wing, 5),
        "chord_mean_m":     round(result.wing.chord_mean, 5),
        "AR":               round(aero.AR, 4),

        # -- Mission (repeated for convenience -- no need to open config/) --
        "V_cruise_ms":      round(mission.V_cruise, 3),
        "V_stall_ms":       round(aero.V_stall, 3),
        ...
        # -- Environment --
        "rho_kg_m3":        round(env.rho, 4),
        "g_m_s2":           round(env.g, 5),
    ...
    with open(path, "w", encoding="utf-8") as f:
        f.write("# AUTO-GENERATED by mass_closure.py -- do not edit by hand.\n")
        f.write("# Source  : src/conceptual_design/mass_closure.py\n")
        f.write("# Inputs  : all config/*.yaml files\n")
        f.write("# Regen   : re-run Section 5 of vbat_conceptual_design.ipynb\n\n")
        yaml.dump(data, f, default_flow_style=False, sort_keys=False)

    print(f"Sizing result written -> {path}")
